...
reassign_indices(CState) ->
    reassign_indices(CState, [], rand:seed_s(exsplus), fun no_log/2).
...
init([]) ->
    schedule_tick(),
    {ok, #state{changes=[], seed=rand:seed_s(exsplus)}}.
...
clear_staged(State) ->
    ...
    State#state{changes=[], seed=rand:seed_s(exsplus)}.
...
do_maybe_force_ring_update(Ring) ->
    case compute_next_ring([], rand:seed_s(exsplus), Ring) of
        {ok, NextRing} ->
...
do_claimant(Node, CState, Log) ->
    do_claimant(Node, CState, [], rand:seed_s(exsplus), Log).
